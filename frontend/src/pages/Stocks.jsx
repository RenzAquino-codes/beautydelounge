...
//         if (Number(form.quantity) <= 0) return showToast("Quantity must be greater than zero.");
//         setIsSaving(true);
//         try {
...
function Stocks() {
    ...
    useEffect(() => {
        fetch(`${API}/api/stocks`, { headers: { "Authorization": `Bearer ${token()}` }})
            .then(res => res.json())
            .then(data => setStocks(data));

        fetch(`${API}/api/categories?type=stock`, { headers: { "Authorization": `Bearer ${token()}` }})
            .then(res => res.json())
            .then(data => setCategories(Array.isArray(data) ? data : []));
    }, []);
    ...
    const handleSave = async () => {
        ...
        try {
            const imageUrl = await uploadImage();
            const finalForm = imageUrl ? { ...form, imageUrl } : form;
            const url = editingItem ? `${API}/api/stocks/${editingItem}` : `${API}/api/stocks`;
            ...
            if (res.ok) {
                const data = await res.json();
                setStocks(editingItem ? stocks.map(s => s._id === editingItem ? data : s) : [...stocks, data]);
                setShowForm(false);
                showToast(editingItem ? "Updated!" : "Added!", "success");
            }
        } finally {
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            await Promise.all(selectedItems.map(id => 
                fetch(`${API}/api/stocks/${id}`, { method: "DELETE", headers: { "Authorization": `Bearer ${token()}` }})
            ));
        ...
        <div className="dashboard-container">
            ...
            <main className="main-content">
                <header className="dashboard-header">
                    ...
                    <div className="filter-controls">
                        <div className="search-container">
                            <input type="text" placeholder="Search inventory..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="search-input" />
                        </div>
                        <div className="category-filter">
                ...
                <div style={{ marginBottom: '15px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', color: '#6b5c45', fontSize: '14px', fontWeight: 500 }} onClick={toggleSelectAll}>
                    {selectedItems.length === filteredStocks.length && filteredStocks.length > 0 ? <FaCheckSquare style={{ color: '#c9a84c', fontSize: '18px' }}/> : <FaSquare style={{ color: '#dcd5c9', fontSize: '18px' }}/>}
                    Select All
                </div>
                ...
                {showForm && (
                    <div className="modal-overlay">
                        <div className="modal">
                            ...
                            <input id="stockImageInput" type="file" accept="image/*" style={{ display: 'none' }} onChange={handleImageChange} />
                    ...
                    {filteredStocks.length > 0 ? (
                        filteredStocks.map(item => (
                            <div key={item._id} className={`stock-card ${selectedItems.includes(item._id) ? 'selected-card' : ''}`} style={{ border: item.quantity < 5 ? '1px solid #e74c3c' : '', position: 'relative' }}>
                                ...
                                <div className="stock-card-image">
                                    {item.imageUrl ? <img src={item.imageUrl} alt={item.name} /> : <div className="stock-no-image">📦</div>}
                                </div>
                                <div className="stock-card-body">
                                    <h3 className="stock-card-name">{item.name}</h3>
                                    <p className="stock-card-category">{item.category}</p>
